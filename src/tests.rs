...
fn rtp_encoding_parameters_and_transceiver_init() {
    ...
    assert_eq!(enc.network_priority(), NetworkPriority::Low);
    enc.set_bitrate_priority(4.0);
    enc.set_network_priority(NetworkPriority::VeryLow);
    enc.set_request_key_frame(true);
    enc.set_num_temporal_layers(Some(2));
    assert_eq!(enc.bitrate_priority(), 4.0);
    assert_eq!(enc.network_priority(), NetworkPriority::VeryLow);
    ...
    let mid = NetworkPriority::Medium;
    assert_eq!(NetworkPriority::from_int(mid.to_int()), mid);

...
- **C ラッパーではポインタ引数の null チェックを原則として行わない**
  - webrtc_c は libwebrtc の薄いラッパーであり、C 側から null が渡されたらそのまま C++ 側へ渡す
  - それによって生じるクラッシュや未定義動作は呼び出し側の責任である
  - ただしデリファレンスが必要な箇所では、デバッグビルドで契約違反を検出できるよう `assert(ptr != nullptr)` を入れる

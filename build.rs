...
fn main() {
    ...
    let lib_path = build_webrtc_c(&webrtc_dir, &target_platform);
...
fn build_webrtc_c(webrtc_dir: &Path, target_platform: &str) -> PathBuf {
...
fn emit_link_directives(lib_path: &Path) {
    ...
    match target_os.as_str() {
        "linux" => {
            for lib in ["stdc++", "m", "dl", "rt", "X11", "pthread"] {

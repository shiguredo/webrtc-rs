# FieldTrialsViewRef に is_disabled を、EnvironmentRef に to_owned を追加する
...
0105 で追加したフィールドトライアル API に以下の 2 つが無く、利用側で実用にならない。
...
- README と `skills/shiguredo-webrtc/SKILL.md` のフィールドトライアルの説明に `is_disabled` と `EnvironmentRef::to_owned` を追加する
